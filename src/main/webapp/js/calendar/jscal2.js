...
Calendar = (function() {
    ...
    l.moveTo = function(aG, aD) {
        ...
        if (az.showTime) {
            this.setHours(aG.getHours()); //todo 删除UTC
            this.setMinutes(aG.getMinutes()) //todo 删除UTC
        }
        if (!az.animation) {
            aD = false
        }
    ...
    function N(D) {
        switch (D) {case"time-hour+":this.setHours(this.getHours() + 1);break;case"time-hour-":this.setHours(this.getHours() - 1);break;case"time-min+":this.setMinutes(this.getMinutes() + this.args.minuteStep);break;case"time-min-":this.setMinutes(this.getMinutes() - this.args.minuteStep);break;default:return
        }
    }
    ...
    function aa(P, at, D) {
        ...
        if (at != 0) {
            ar = new Date(P.date);
            ar.setDate(1);
            switch (at) {case"-Y":case -2:ar.setFullYear(ar.getFullYear() - 1);break;case"+Y":case 2:ar.setFullYear(ar.getFullYear() + 1);break;case"-M":case -1:ar.setMonth(ar.getMonth() - 1);break;case"+M":case 1:ar.setMonth(ar.getMonth() + 1);break
            }
        } else {
            ar = new Date()
        }
    ...
    function E(ar) {
        ...
        if (P) {
            ...
            if (/^(time-(hour|min))/.test(D)) {
                switch (RegExp.$1) {case"time-hour":this.setHours(this.getHours() + au);break;case"time-min":this.setMinutes(this.getMinutes() + this.args.minuteStep * au);break
                }
                an(ar)
            } else {
                if (/Y/i.test(at)) {
    ...
    y.parseDate = function(au, D, aw) {
        ...
        return aB && P && aD ? new Date(aB, P - 1, aD, av, ar, aC) : null //todo 删除Date.UTC(),修改成获取本机时间
    };
